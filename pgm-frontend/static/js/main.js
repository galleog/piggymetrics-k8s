...
function initAccount(account) {
    user = new User(account.name, account.updateTime, account.saving.moneyAmount.currency, account.note);
    savings = new Savings(account.saving.moneyAmount.amount, account.saving.deposit,
        account.saving.capitalization, account.saving.interest);

    let i = 1;
    account.items.filter(item => item.type === incomeType)
        .forEach(item => AddIncome(i++, item.title, item.icon, item.moneyAmount.currency, item.period, item.moneyAmount.amount));
    let j = 1;
    account.items.filter(item => item.type === expenseType)
        .forEach(item => AddExpense(j++, item.title, item.icon, item.moneyAmount.currency, item.period, item.moneyAmount.amount));
}
...
function sanitize(obj) {
    ...
    $.each(obj, function (index, item) {
        $.each(item, function (key, value) {
            obj[index][key] = escapeHtml(value);
        });
    });
...
function initGreetingPage() {
    ...
    $("#lefttitle").prepend(escapeHtml(user.login));
...
function greetingPageAgain() {
    ...
    $("#lefttitle").append(escapeHtml(user.login) + '<span class="bluetext"> metrics</span>');
}
...
function jsonDataSave() {
    if (global.savePermit) {
        $.ajax({
                ...
                data: JSON.stringify({
                    note: user.notes,
                    items: $.map(incomes, function (value) {
                        return {
                            type: incomeType,
                            title: value.title,
                            period: value.period,
                    ...
                    }).concat($.map(expenses, function (value) {
                        return {
                            type: expenseType,
                            title: value.title,
                            period: value.period,
