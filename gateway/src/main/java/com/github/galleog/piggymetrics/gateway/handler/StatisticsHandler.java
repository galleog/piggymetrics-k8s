...
public class StatisticsHandler {
    ...
    public Mono<ServerResponse> getCurrentAccountStatistics(ServerRequest request) {
        return getStatistics(request.principal().map(Principal::getName));
    }

    /**
     * Gets statistical metrics for the account specified in a path variable.
     *
     * @param request the server request
     * @return a list of data points for the specified account
     */
    public Mono<ServerResponse> getStatisticsByAccountName(ServerRequest request) {
        return getStatistics(Mono.just(request.pathVariable("name")));
    }

    private Mono<ServerResponse> getStatistics(Mono<String> nameMono) {
        Mono<StatisticsServiceProto.ListDataPointsRequest> request = nameMono.map(name ->
